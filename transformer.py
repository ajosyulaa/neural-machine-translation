...
class EncoderLayer(nn.Module):
  ...
  def forward(self, inp, mask):
    ...
    # attention_out = inp + self.attention(self.layernorm_attention(inp,inp,inp), mask)
...
class Encoder(nn.Module):
  def __init__(self, params):
    ...
    self.layers = [EncoderLayer(params) for l in range(self.nlayers)]
...
class DecoderLayer(nn.Module):
  ...
  def forward(self, inp, encoder_out, mask1, mask2):
    ...
    # masked_attention_out = inp + self.attention(self.layernorm_masked_attention(inp,inp,inp), mask1)
    ...
    # attention_out = masked_attention_out + self.attention(self.layernorm_attention(masked_attention_out, encoder_out, encoder_out), mask2)
...
class Decoder(nn.Module):
  def __init__(self, params):
    ...
    self.nlayers = params['nlayers']
    self.layers = [DecoderLayer(params) for l in range(self.nlayers)]
    self.layers = nn.ModuleList(self.layers)
    self.norm = LayerNorm(params['d_model'])
...
class Transformer(nn.Module):
  ...
  def forward(self, src_inp, tgt_inp, pad_idx):
    ...
    src_mask = (src_inp != pad_idx).unsqueeze(-2).unsqueeze(-2) # bsz x 1 x 1 x seq_len1
    tgt_pad_mask = (tgt_inp != pad_idx) # bsz x seq_len2
    tgt_future_mask = torch.ones(tgt_inp.size(),dtype = torch.uint8).tril().cuda() # bsz x seq_len2
    tgt_mask = (tgt_pad_mask & tgt_future_mask).unsqueeze(-2).unsqueeze(-2) # bsz x 1 x 1 x seq_len2
...
MIN_FREQ = 1
DE_TEXT.build_vocab(train, val, min_freq = MIN_FREQ, max_size=50000)
EN_TEXT.build_vocab(train, val, min_freq = MIN_FREQ, max_size=50000)
pad_idx = EN_TEXT.vocab.stoi['<pad>']

# print(EN_TEXT.vocab.stoi['<pad>'])
# print(len(EN_TEXT.vocab.stoi))
# print(DE_TEXT.vocab.stoi['<pad>'])
# print(len(DE_TEXT.vocab.stoi))
...
train_iter = MyIterator(train, batch_size=512, device = device,
                        repeat=False, sort_key= lambda x:
                        (len(x.German), len(x.English)),
                        batch_size_fn=batch_size_fn, train=True,
                        shuffle=True)
val_iter2 = MyIterator(val, batch_size=512, device = device,
                        repeat=False, sort_key= lambda x:
                        (len(x.German), len(x.English)),
                        batch_size_fn=batch_size_fn, train=True,
                        shuffle=True)
val_iter = MyIterator(val, batch_size=512, device = device,
...
#             print(preds.size(),targets.size())
        ...
        print('epoch: %d, dev_loss: %0.2f' %(epoch, calculate_loss(val_iter,net)))
        model_name = "transformer_" + epoch 
        torch.save(transformer_obj, model_name)
...
val_iter = Iterator(val, batch_size=10, device = device,repeat=False, train=False,sort = False,shuffle=False)
  ...
  for it, batch in enumerate(data_iter):
    ...
    for i in range(indices.size()[0]):
      ...
      for j in range(indices.size()[1]):
        if(indices[i,j] == EN_TEXT.vocab.stoi['<eos>']):
          break
        sentence.append(EN_TEXT.vocab.itos[int(indices[i,j])])
      sentence_joined = " ".join(sentence)
#       if it < num_batches - 1 or i < indices.size()[0] - 1:
      sentence_joined += '\n'
#       print(sentence_joined)
      if i != 0 or it != 0:
        f.write(sentence_joined)
#       sentences.append(sentence_joined)
  f.close()
